...
(function (root, factory) {
  ...
  class OCEngine {
    constructor(opts) {
      ...
      this.startTurn = (opts.startTurn != null ? opts.startTurn | 0 : null);   // 선공 고정(재현·대조용). 없으면 매 판 무작위
    ...
    _newGame() {
      ...
      this.draw = deck;
      /* 선공을 매 판 뽑는다 — 늘 0번 자리가 먼저 내던 것을 고쳤다.
         전원 AI 실측에서 0번 자리 승률이 3인 41.5%(기대 33.3%) · 4인 30.3%(기대 25%)로
         두 인원 모두 같은 방향으로 치우쳤다. 온라인에선 방장이 그만큼 유리해진다. */
      this.dir = 1; this.attack = 0; this.reqSuit = null;
      this.turn = (this.startTurn != null ? this.startTurn : Math.floor(this.rng() * this.N));
