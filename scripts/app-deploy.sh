...
#   npm run app:deploy
#
# 🟢 2026-08-12 OTA 전환: capacitor.config.json에 server.url(github.io)을 넣어,
#   앱이 매 실행 시 **라이브 웹(github.io)을 원격 로드**한다(데스크톱 Electron과 동일 방식).
#   → 게임을 고쳐 웹에 배포하면(Pages) **앱은 다음 실행에 자동으로 최신**이 된다(OTA).
#   이 스크립트로 APK를 다시 만드는 건 **네이티브 설정/플러그인이 바뀔 때만** 필요하다
#   (콘텐츠 업데이트엔 불필요 — 그게 OTA의 요점).
#   ⚠️ 원격 로드라 앱은 실행 시 네트워크가 필요하다(옛 번들 오프라인은 없어짐 — 필요 시 SW로 복원 가능).
#
