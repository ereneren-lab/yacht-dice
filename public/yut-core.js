  ...
  class YutEngine {
    ...
    _orderThrow(pid, power) {
      ...
      this._orderTimer = setTimeout(() => {
        ...
        this._emit(); this._maybeAI();
      }, this.orderTie ? 2800 : 2000);   // 결과를 넉넉히 읽을 시간(동점 재대결은 더 길게)
      return;
    }
    _orderFinish() {
    ...
    doMove(pid, pieceId, pendingIndex, dir, carry, ownerSeat) {
      ...
      // ⚠️ 같은 칸 판정은 node만 본다(route는 제외). 중앙칸(22)은 sc5·sc10이, 15~19·29칸은
      //    outer·sc5가 겹쳐서, 지름길로 온 말과 바깥길로 온 말이 '같은 칸에 업혔는데' route가 다르다.
      //    route까지 같아야 묶으면 다른 route 말이 낙오된다(실제로 터짐). 업힌 말은 캐리어 route를 따라간다.
      const mates = this.teamMode ? this.players.filter(p => p.team === pl.team) : [pl];
      const sameCell = (pc.out && !pc.done)
        ? mates.reduce((a, tp) => a.concat(tp.pieces.filter(x => !x.done && x.out && x.node === pc.node)), [])
        : [pc];
