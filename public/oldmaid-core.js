...
(function (root, factory) {
  ...
  class OMEngine {
    ...
    _newGame() {
      ...
      this.lastAction = null; this.lastPair = null; this.lastJoker = null; this.lastSafe = null;
      /* 라운드로빈 분배 — **첫 장을 받는 자리도 매 판 무작위**로 돌린다 (v1.238).
         53장이라 인원에 따라 한 자리가 1장 더 받는데(4인이면 14/13/13/13),
         늘 0번부터 돌리면 **그 한 장이 항상 같은 자리에** 가서 자리 편향이 생긴다.
         카드를 빼서 균등하게 맞출 수는 없다 — 총합은 홀수(조커 1 + 짝 N개)여야 해서
         짝수 인원에선 균등 분배가 애초에 불가능하고, 억지로 빼면 짝이 안 맞는 카드가
         남아 '술래 없는 판'이 나온다(실제로 실험해서 확인했다). */
      let s = Math.floor(this.rng() * this.N);
      ...
      /* 첫 차례도 매 판 무작위 (v1.238). 위의 무작위 분배와 **둘 다** 있어야 고르게 된다.
         실측(8,000판 × 인원별) 술래 확률 편차:
           고정시작·고정분배(원래) 3인 20.9pt·5인 18.4pt (5인 1번 자리는 남의 1/3.5)
           무작위시작만            3인  6.4pt·4인  8.0pt  ← 4인은 오히려 나빠진다
           둘 다 무작위            3인  1.7pt·4인  1.2pt·5인 0.8pt (최대 1.05배)
         규칙은 하나도 안 바꿨다 — 누구부터 돌리고 누가 먼저 하느냐만 돌린 것이라
         실물 게임과 같다. */
      this._beginTurn(this._firstActive(Math.floor(this.rng() * this.N)));
    }
