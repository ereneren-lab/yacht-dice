...
- 위치: **`~/yacht-dice`** 가 유일한 저장소. 버전 **v1.154.0**.
...
## 2026-07-22 세션 (v1.154) — 채팅 2인 실측 + 자리 소유권 버그
...
3. ~~채팅 2인 실측~~ — **완료(v1.154)**. 남은 미해결 한 가지: **새 탭이 초대 링크로 처음 열릴 때**는 sessionStorage가 비어 localStorage로 폴백하므로, 첫 탭이 방에 있는 상태에서 같은 브라우저에 새 탭을 열면 여전히 자리를 가져갈 수 있다(기존과 동일, 루프는 안 남). 막으려면 BroadcastChannel로 "이 자리 쓰는 탭 있나?" 물어보고 폴백을 건너뛰면 된다.
