...
- 위치: **`~/yacht-dice`** 가 유일한 저장소. 버전 **v1.156.0**.
...
4. **남은 UX 후보(직접 보고 남겨둔 것)** — ① 윷 데스크톱 1440×900에서 판이 작고 아래 1/3이 빈 공간 ② 윷 첫 판 튜토리얼 코치마크가 모바일에서 결과·버튼을 가림(1회성) ③ 너클본즈 첫 진입 규칙 모달이 모바일에서 잘림.
