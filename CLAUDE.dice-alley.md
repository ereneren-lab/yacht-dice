...
**⚠️ 지금 계측은 꺼져 있다.** 2026-07-22 '계측 보류' 결정으로 **13개 페이지 전부에서
`analytics.js` 로딩이 주석 처리**되어 있다(파일과 `AL.*` 호출부는 그대로).
그래서 대시보드에 아무것도 안 찍힌다 — "방문자가 없다"가 아니라 "안 재고 있다"이다.
켤 때는 각 HTML의 '계측 보류' 주석 블록에서 두 줄을 꺼내면 된다.

**검증 상태 (v1.247).** `verify-analytics.js` 30개 단언 통과.
`게임시작`·`허브_카드클릭`은 실제 브라우저에서 발화를 봤다.
**`1판완료`도 이제 실제 발화를 확인했다** — `npm run test:doneevent`가 계측을 테스트에서만
주입해 13종 전부에서 `[AL] 1판완료`와 **게임 이름까지** 단언한다.
그 과정에서 `GAMES` 표에 5종만 있어 **나머지 8종이 '허브'로 집계**되던 버그를 잡았다.
