...
- **같은 브라우저의 두 탭은 localStorage를 공유한다.** 두 탭으로 2인을 흉내 낼 때
  ...
  새 탭이 초대 링크로 처음 열릴 땐 sessionStorage가 비어 폴백하므로, v1.155에서 `BroadcastChannel`로
  "이 자리 쓰는 탭 있냐"를 물어 자동 rejoin을 포기한다. **가드가 살아있는지 반증**하려면
  `Page.addScriptToEvaluateOnNewDocument`로 `delete window.BroadcastChannel`을 주입해 옛 동작을 재현해 보면 된다.
