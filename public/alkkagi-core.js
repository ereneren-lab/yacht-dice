  ...
  class AlkkagiEngine {
    constructor(opt) {
      ...
      if (this.rule === 'wind') {
        ...
        const WIND_MAG = 9;
        const _rng = opt.rng || Math.random;   // this.rng는 아래에서 대입된다(아직 없음)
        const dir = opt.windDir != null ? opt.windDir : (_rng() < 0.5 ? -1 : 1);
        this.windDir = dir;
    ...
    _naiveFlick(seat) {   // easy가 가끔 섞어 쓰는 옛 방식(초보에게 이길 여지 남김)
      ...
      const me = mine[Math.floor(this.rng() * mine.length)];
      let best = opps[0], bestD = Infinity;
      for (const o of opps) { const d = (o.x - me.x) ** 2 + (o.y - me.y) ** 2; if (d < bestD) { bestD = d; best = o; } }
      const angle = Math.atan2(best.y - me.y, best.x - me.x) + (this.rng() - 0.5) * 0.35;
      return { stoneId: me.id, angle, power: 0.55 + this.rng() * 0.35 };
    }
    ...
    _pickAiFlick(seat, diff) {
      ...
      if (diff === 'easy' && this.rng() < 0.5) return this._naiveFlick(seat);
        ...
        for (const o of near) {
          ...
          for (const off of offsets) {
            for (const pw of powers) {
              ...
              if (score > bestScore || (score === bestScore && this.rng() < 0.5)) { bestScore = score; best = flick; }
            }
          }
        }
      }
      ...
      return { stoneId: best.stoneId, angle: best.angle + (this.rng() - 0.5) * jitter, power: best.power };
    }
