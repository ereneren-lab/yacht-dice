...
## 5·6파 `sheetB/` — sad·angry, 10/13 (2026-08-20)

`star`를 권했는데 **B시트(sad·angry·cheer)로 오셨다.** 3-업 시트 10장 = 30컷.
`cheer`는 이미 낱장으로 들어가 있어(더 고해상도·전신) **버리고 sad·angry만 20장 썼다.**

**시트 자르기는 10장 전부 한 번에 갈렸다** — 폭 편차 최대 1.35배 이내, 붙은 것 없음.
프롬프트의 `LARGE EMPTY GAPS … must not touch or overlap`가 먹혔다(8/20 오전엔 8장 중 2장이 붙었었다).

### ⚠️ 프레이밍은 캐릭터마다 따로 잡아야 했다

`cheer`는 우승 화면(전신)이라 쉬웠는데, **sad·angry는 캐릭터 카드(74~150px)에 뜨고
그 자리의 베이스는 얼굴로 바짝 잘린 `{id}.png`** 다. 시트 컷은 몸까지 나와서 그냥 넣으면
표정을 쏠 때 **확 줌아웃**된다.

베이스와 **채움비(불투명 픽셀 비율)**가 가장 가까운 설정을 자동으로 골랐다:

| id | 설정 |
|---|---|
| `tiger` `rabbit` `bear` `fox` `dokkaebi` | `--bg none` (원래 상반신이라 그대로) |
| `yutgarak` `bokjumeoni` `chorong` `jeoseung` | `--bg none --bust 0.7 --tight` |
| `haetae` | `--bg none --bust 0.8 --tight` |

⚠️ **채움비 기준은 만능이 아니다.** 해태는 갈기가 커서 자동 선택(0.5)이 **얼굴을 잘라냈다** —
채움비는 맞는데 정작 얼굴이 없었다. 눈으로 보고 0.8로 올렸다.
**덩치 큰 비-얼굴 부위(갈기·꼬리·모자)가 있는 캐릭터는 수치만 믿지 말 것.**

### 통과했지만 어긋난 둘 (받아들였다)

| id | 무엇 |
|---|---|
| `dokkaebi` | 베이스는 **붉은 민소매 조끼**인데 남색/흰 도포로 나왔다. 뿔·초록 피부·방울·방망이는 유지 |
| `yutgarak` | 베이스의 **빨간 머리띠**가 없다. 얼굴·붉은 X·나뭇결은 유지 |

둘 다 **이미 배포된 `cheer`와 같은 방향으로 어긋나 있어**, 표정 세트 안에서는 서로 일관된다.
여기서 되돌리면 `cheer`까지 다시 뽑아야 한다. 다음 파에서 셋을 함께 맞추는 편이 싸다.

### 안 온 3종

`jangseung` · `sansin` · `yong` — B시트가 없다. 이 셋은 아직 `cheer`만 있다.
