...
function startEngine(room){
  ...
  if (room.game === 'kb'){
    ...
    const onState = ()=> broadcast(room, { t:'state', state: room.engine.serialize() });
    room.engine = new AlkkagiEngine({ aiFast:!!room.aiFast, players, preset:(['mini','standard','battle'].includes(room.preset)?room.preset:'standard'), surface:room.surface, specials:room.specials, aiMs:900, onState });
  } else {
    const onState = (s)=> broadcast(room, { t:'state', state:s });
